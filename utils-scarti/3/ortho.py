...
def enu_to_canvas(positions_en, ortho_result: "OrthoResult") -> np.ndarray:
    """Mappa coordinate ENU mondo a pixel canvas usando i parametri di `OrthoResult`.

    Inversa di `_affine_image_to_canvas` ma sui punti mondo, non sui pixel sorgente:
        X = (E - east_min) / gsd
        Y = (north_max - N) / gsd       (Y canvas cresce verso sud)

    Accetta array shape (N, 2) o (N, 3); l'eventuale terza colonna (Up) è ignorata.
    """
    p = np.atleast_2d(np.asarray(positions_en, dtype=np.float64))
    X = (p[:, 0] - ortho_result.east_min) / ortho_result.gsd
    Y = (ortho_result.north_max - p[:, 1]) / ortho_result.gsd
...
def render_ortho(
    ...
    *,
    ...
    _diagnose_seam_residuals(images, M_use)
...
def _diagnose_seam_residuals(images, M_use, *, n_features=2000, min_inliers=20):
    """Stampa scollamento residuo in pixel canvas tra coppie consecutive.

    Per ogni coppia (i, i+1) fa ORB+RANSAC per trovare i match condivisi, mappa
    gli inlier dal frame sorgente al canvas via M_use[i] e M_use[i+1] e misura
    la distanza tra le due proiezioni. Se la geometria fosse perfetta dovrebbe
    essere ~0; tipicamente fluttua di qualche pixel per errori sub-grado yaw +
    sub-metro GPS.

    Diagnostica per decidere se serve un refine, un blender migliore o nulla.
    """
    ...
    for i in range(len(images) - 1):
        gA = cv2.cvtColor(images[i], cv2.COLOR_BGR2GRAY)
        gB = cv2.cvtColor(images[i + 1], cv2.COLOR_BGR2GRAY)
        kpA, desA = orb.detectAndCompute(gA, None)
        kpB, desB = orb.detectAndCompute(gB, None)
        if desA is None or desB is None or len(desA) < min_inliers or len(desB) < min_inliers:
            n_failed += 1
            continue
        ...
        inl = inliers.ravel().astype(bool)
        K = int(inl.sum())
        ptsA_h = np.hstack([ptsA[inl], np.ones((K, 1), dtype=np.float32)])
        ptsB_h = np.hstack([ptsB[inl], np.ones((K, 1), dtype=np.float32)])
        canvasA = ptsA_h @ np.asarray(M_use[i]).T
        canvasB = ptsB_h @ np.asarray(M_use[i + 1]).T
        all_diff.append(np.linalg.norm(canvasA - canvasB, axis=1))
        n_ok += 1

    if not all_diff:
        print("[ortho-diag] Nessuna coppia con match sufficienti.")
        return
    diffs = np.concatenate(all_diff)
    print(
        f"[ortho-diag] Scollamenti canvas tra coppie consecutive: "
        f"mediana {np.median(diffs):.2f}px, p90 {np.percentile(diffs, 90):.2f}px, "
        f"max {diffs.max():.2f}px. Coppie OK: {n_ok}/{len(images) - 1}"
        f"{f' (fallite: {n_failed})' if n_failed else ''}."
    )
